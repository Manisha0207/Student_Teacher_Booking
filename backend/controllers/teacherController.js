...
exports.getAllPendingStudents = catchAsync(async (req, res, next) => {
    const students = await Appointment.find({
        ...
        .populate({ path: "students.studentId", select: "_id name department email" })
        .select("-students.approved -students._id -sendBy");
...
// ✅ Get all appointments for logged-in teacher/admin
exports.getAllAppointments = catchAsync(async (req, res) => {
    const appointments = await Appointment.find({ sendBy: req.user.email });
    res.status(200).json({ appointments });
});
...
// ✅ Approve a student's appointment (no email sending now)
exports.approveAppointment = catchAsync(async (req, res) => {
...
// ✅ Disapprove a student's appointment (no email sending now)
exports.dissapproveAppointment = catchAsync(async (req, res) => {
    await Appointment.findOneAndUpdate(
        { _id: req.params.id },
        {
    ...
    res.status(200).json({ message: "Student removed from appointment" });
});
